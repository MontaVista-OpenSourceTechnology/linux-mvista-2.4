...
static void __iomem *iproc_pcie_base = NULL;

int test_msi = -1;
module_param(test_msi,int,0);
...
static void * __iomem ob_addr = NULL;
...
static ep_intr_handler handlers[SOC_PCIE_EP_MAX_IRQ] =
	{ ep_intr0_handler, ep_intr1_handler };
...
msi_interrupt_generate(u32 fn, u32 mvec)
{
	...
	printk("OARR_FUNC0_MSI_PAGE : 0x%x\n", value);
	value |= 0x1;
...
pcie_ep_init(void)
{
	...
	if (test_msi == 1) {
		printk("Testing pcie ep generate msi interrupt to eHost\n");
